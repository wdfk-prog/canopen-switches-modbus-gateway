...
#define ucMB_RTU_PORT	 (mbrtuport2)
/*波特率设置*/
...
/* 指向互斥量的指针 */
extern rt_mutex_t mb_slave_mutex;
/* MODBUS函数 */
...
/* MODBUS结构体 */
static _CONST MB_RTU_PORT mbrtuport2 = 
{
	MBPortSerialInit,
	MBPortTimersInit, 
	MBPortSerialGetByte,
...
static void modbus_thread(void* p)
{
  ...
  while(1)
  {
    ...
    if (result == RT_EOK)
    {
        ...
        {
          if(msg.size > MB_SER_PDU_SIZE_MAX)
          {
            LOG_W("The receive length is greater than the cache");
            continue;//跳过当前循环
          }
...
static int Modbus_Slave2_Init(void)
{
    ...
    struct rx_msg msg;
    /* 初始化消息队列 */
    rx_mq = rt_mq_create("mbs1ave2",
                         sizeof(struct rx_msg),    /* 一条消息的最大长度 */
                         BSP_UART2_RX_BUFSIZE,     /* 存放消息的缓冲区大小 */
                         RT_IPC_FLAG_FIFO); 
