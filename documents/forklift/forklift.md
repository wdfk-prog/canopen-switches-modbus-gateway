...
| 保持寄存器地址 | 数据类型 | 参数范围 | 默认值 | 备注                            |
| -------------- | -------- | -------- | ------ | ------------------------------- |
| 49D            | float    | 0~20000  | 600    | 转向电机[1]速度输入 单位:0.1RPM |
| 50D            | float    | 0~20000  | 600    | 转向电机[2]速度输入 单位:0.1RPM |
| 51D            | float    | 0~20000  | 600    | 转向电机[3]速度输入 单位:0.1RPM |
| 52D            | float    | 0~20000  | 600    | 转向电机[4]速度输入 单位:0.1RPM |
...
- 速度范围默认-2000~2000RPM，超出速度范围将只能运动到最大速度范围，并反馈速度超出范围标志。[2.3.3 行走电机速度超出范围标志查询](#2.3.3 行走电机速度超出范围标志查询)。可通过设置速度范围调整[2.2.2 行走电机速度范围设置](#2.2.2 行走电机速度范围设置)。

| 保持寄存器地址 | 数据类型 | 参数范围     | 默认值 | 备注                            |
| -------------- | -------- | ------------ | ------ | ------------------------------- |
| 61D            | uint16   | -20000~20000 | 600    | 行走电机[1]速度输入 单位:0.1RPM |
| 62D            | uint16   | -20000~20000 | 600    | 行走电机[2]速度输入 单位:0.1RPM |
| 63D            | uint16   | -20000~20000 | 600    | 行走电机[3]速度输入 单位:0.1RPM |
| 64D            | uint16   | -20000~20000 | 600    | 行走电机[4]速度输入 单位:0.1RPM |

#### 2.2.2 行走电机速度范围设置

- 设置电机速度范围，单位:RPM。默认值最大速度2000,最小速度为0(不可修改)。
- 超出速度范围将只能运动到最大速度范围，并反馈速度超出范围标志。[2.2.3 转向电机角度超出范围标志查询](#2.3.3 转向电机角度超出范围标志查询)。

| 保持寄存器地址 | 数据类型 | 参数范围 | 默认值 | 备注                         |
| -------------- | -------- | -------- | ------ | ---------------------------- |
| 65D            | uint16   | 无       | 2000   | 行走电机[1]最大速度 单位:RPM |
| 66D            | uint16   | 无       | 2000   | 行走电机[2]最大速度 单位:RPM |
| 67D            | uint16   | 无       | 2000   | 行走电机[3]最大速度 单位:RPM |
| 68D            | uint16   | 无       | 2000   | 行走电机[4]最大速度 单位:RPM |
...
| 输入寄存器地址 | 数据类型 | 参数范围 | 默认值 | 备注                            |
| -------------- | -------- | -------- | ------ | ------------------------------- |
| 61D            | int16    | 无       | 0      | 行走电机[1]反馈速度 单位:0.1RPM |
| 62D            | int16    | 无       | 0      | 行走电机[2]反馈速度 单位:0.1RPM |
| 63D            | int16    | 无       | 0      | 行走电机[3]反馈速度 单位:0.1RPM |
| 64D            | int16    | 无       | 0      | 行走电机[4]反馈速度 单位:0.1RPM |

#### 1.3.3 行走电机速度超出范围标志查询
...
| 离散线圈输入寄存器地址 | 默认值 | 备注                        |
| ---------------------- | ------ | --------------------------- |
| 11D                    | 0X00   | 行走电机[1]速度超出范围标志 |
| 12D                    | 0X00   | 行走电机[2]速度超出范围标志 |
| 13D                    | 0X00   | 行走电机[3]速度超出范围标志 |
| 14D                    | 0X00   | 行走电机[4]速度超出范围标志 |
...
| 输入寄存器地址 | 数据类型 | 参数范围 | 默认值 | 备注                |
| -------------- | -------- | -------- | ------ | ------------------- |
| 65D            | uint16   | 无       | 0      | 行走电机[1]停止代码 |
| 66D            | uint16   | 无       | 0      | 行走电机[2]停止代码 |
| 67D            | uint16   | 无       | 0      | 行走电机[3]停止代码 |
| 68D            | uint16   | 无       | 0      | 行走电机[4]停止代码 |
...
| 40D     | uint16   | 无               | 0        | 保留                            |
|         |          | 转向电机区域     |          |                                 |
| 41D~42D | float    | -180~180         | 当前角度 | 转向电机[1]角度输入 单位:0.001° |
| 43D~44D | float    | -180~180         | 当前角度 | 转向电机[2]角度输入 单位:0.001° |
| 45D~46D | float    | -180~180         | 当前角度 | 转向电机[3]角度输入 单位:0.001° |
| 47D~48D | float    | -180~180         | 当前角度 | 转向电机[4]角度输入 单位:0.001° |
| 49D     | float    | 0~20000          | 600      | 转向电机[1]速度输入 单位:0.1RPM |
| 50D     | float    | 0~20000          | 600      | 转向电机[2]速度输入 单位:0.1RPM |
| 51D     | float    | 0~20000          | 600      | 转向电机[3]速度输入 单位:0.1RPM |
| 52D     | float    | 0~20000          | 600      | 转向电机[4]速度输入 单位:0.1RPM |
